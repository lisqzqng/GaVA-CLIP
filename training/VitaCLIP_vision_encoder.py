...
class CLIPVisionEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = torch.cat([self.cls_token.view(1, 1, -1).repeat(x.size(0), 1, 1), x], dim=1) # cls_token per-frame
        ...
        if self.use_global_prompts:
            for i, blk in enumerate(self.blocks): # 12 layer transformer
                global_prompts = self.global_prompts[i].expand(B*T, -1, -1)

                x = torch.cat((x[:, :1, :], global_prompts, x[:, 1:, :]), dim=1)
                x, summary = blk(x)
                x = torch.cat((x[:, :1, :], x[:, self.num_global_prompts+1:, :]), dim=1) # exclude the global_prompts
        else:
        ...
        return cls_x, summary

if __name__=='__main__':
    # load the pre-trained model and check the hyperparams `patch_embed` #
    clip_path = 'pretrained/clip_pretrained.pth'
    model_path = 'pretrained/ckpt_k400.pth'
    import torch
    # load the state dictionaly from `pth` file
    model_state_dict = torch.load(model_path)['model']
    model_state_dict_visual = {k.replace('module.visual.',''): v for k, v in model_state_dict.items() if k.startswith('module.visual')}
    clip_state_dict = torch.load(clip_path)
    clip_state_dict_visual = {k.replace('visual.',''): v for k, v in clip_state_dict.items() if k.startswith('visual')}
    pass