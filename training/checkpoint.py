...
def save_checkpoint(
    ...
    # save the per-class text features
    text_features = model.module.text_features.detach().cpu()
    to_save['text_features'] = text_features
    if is_best:
        torch.save(to_save, os.path.join(args.checkpoint_dir, f'{name}-best.pth'))
    else:
        torch.save(to_save, os.path.join(args.checkpoint_dir, f'{name}-{next_step}.pth'))
