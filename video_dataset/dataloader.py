...
from .dataset import VideoDataset, DummyDataset, MemoryDataset, DummyMemoDataset
...
def setup_arg_parser(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--val_list_path', type=str, default='',
    ...
    parser.add_argument('--eval_list_path', type=str, default='',
                        help='path to evaluation data list')
    parser.add_argument('--eval_data_root', type=str, default='',
                        help='evaluation samples root directory')
    ...
    parser.add_argument('--num_temporal_views', type=int, default=10,
...
def create_val_loader(args: argparse.Namespace) -> torch.utils.data.Dataset:
    ...
    loader = torch.utils.data.DataLoader(
        dataset, sampler=sampler, batch_size=args.batch_size, drop_last=False,
        num_workers=args.num_workers, pin_memory=False,
    )

    return loader

def create_eval_dataset(args: argparse.Namespace) -> torch.utils.data.Dataset:
    assert args.dummy_dataset is False

    return VideoDataset(
        list_path=args.eval_list_path,
        ...
        **_parse_mean_and_std(args),
        is_train=False,
        num_folds=args.nfold,
        cls_type=args.type,
        add_nte=False,
    )

def create_eval_loader(args: argparse.Namespace) -> torch.utils.data.Dataset:
    dataset = create_eval_dataset(args)
    rank, world_size = (0, 1) if not dist.is_initialized() else (dist.get_rank(), dist.get_world_size())
...
def create_memory_dataset(args: argparse.Namespace) -> torch.utils.data.Dataset:
    if args.use_support_memory:
        return MemoryDataset(
            data_path=args.memory_data_path,
            cls_type=args.type.split('_')[0],
            batch_size=args.mem_batch_size,
            for_zero_shot=args.for_zero_shot,
        )
    else:
        return DummyMemoDataset(
            batch_size=args.mem_batch_size,
            embed_size=args.embed_dim,
        )
    
def create_memory_loader(args: argparse.Namespace, resume_step: int = 0) -> torch.utils.data.Dataset:
    dataset = create_memory_dataset(args)
    
    rank, world_size = (0, 1) if not dist.is_initialized() else (dist.get_rank(), dist.get_world_size())

    assert args.batch_size % world_size == 0
    batch_size_per_gpu = args.mem_batch_size // world_size

    # manually create a step-based sampler
    sampler = []
    while len(sampler) * len(dataset) < args.num_steps * args.mem_batch_size:
        g = torch.Generator()
        g.manual_seed(len(sampler))
        indices = torch.randperm(len(dataset), generator=g)
        sampler.append(indices)
    sampler = torch.cat(sampler, dim=0)[:args.num_steps * args.mem_batch_size].view(args.num_steps, args.mem_batch_size)
    sampler = sampler[resume_step:, batch_size_per_gpu * rank: batch_size_per_gpu * (rank + 1)].flatten().tolist()

    loader = torch.utils.data.DataLoader(
        dataset, sampler=sampler, batch_size=batch_size_per_gpu, shuffle=False,
        num_workers=args.num_workers, pin_memory=False, drop_last=True,
    )
